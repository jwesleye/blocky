...
[`docs/PRD.md` §10 (Non-Functional Requirements)](./PRD.md#10-non-functional-requirements).
...
**Budget:** p95 frame time ≤ 17 ms (≈ 60 fps) while orbiting a 2 000-brick build on
...
**Stress fixture:** 2 000 bricks placed layer-by-layer across a 32×32 stud grid,
generated deterministically by the same algorithm as `scripts/gen-stress-build.ts`
(checked-in canonical artefact: `fixtures/stress-build-2k.json`).
...
- p95 frame time ≤ **17 ms** (≈ 60 fps) across the collapse animation window.
- Synchronous collapse computation for ~300 bricks must complete in < 50 ms
...
**Stress fixture:** 300 bricks — one 1×1 base with a 2×4 overhang (unbalanced) plus
~298 floating bricks with mixed part IDs and all four rotations.
...
**Budget:** all §10 budgets above must pass on **Chrome/Edge, Firefox, and
