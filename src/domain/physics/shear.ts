...
function getOverhangDirection(component: PlacedBrick[]): Direction2D {
  ...
  if (com.x < minX) candidates.push({ magnitude: minX - com.x, direction: { x: -1, z: 0 } })
  if (com.x > maxX) candidates.push({ magnitude: com.x - maxX, direction: { x: 1, z: 0 } })
  if (com.z < minZ) candidates.push({ magnitude: minZ - com.z, direction: { x: 0, z: -1 } })
  if (com.z > maxZ) candidates.push({ magnitude: com.z - maxZ, direction: { x: 0, z: 1 } })

  candidates.sort((a, b) => b.magnitude - a.magnitude)
  return candidates[0]?.direction ?? { x: 0, z: 0 }
}

function getOutermostProjection(brick: PlacedBrick, direction: Direction2D): number {
  ...
  return projections.length > 0 ? Math.max(...projections) : Number.NEGATIVE_INFINITY
}

function isConnected(bricks: PlacedBrick[]): boolean {
  if (bricks.length <= 1) return true
  return connectedComponents(buildConnectionGraph(bricks, PART_CATALOG)).length === 1
}
...
function sortByShearPriority(component: PlacedBrick[]): PlacedBrick[] {
  ...
  return [...component].sort((a, b) => {
    ...
    const projectionDelta =
      getOutermostProjection(b, direction) - getOutermostProjection(a, direction)
...
export function findShearRegion(component: PlacedBrick[]): ShearRegion {
  ...
  for (const brick of candidates) {
    ...
    const remainder = component.filter((candidate) => !shearIds.has(candidate.id))
