...
test.describe('interactions', () => {
  test('places, rotates, deletes, and triggers collapse with undo', async ({ page }) => {
    await page.goto('/')
    ...
    const initialRot = await page.evaluate(() => window.__blockyCursorStore!.getState().rot)
    expect(initialRot).toBe(0)

    await page.keyboard.press('R')

    const newRot = await page.evaluate(() => window.__blockyCursorStore!.getState().rot)
    ...
    await page.evaluate(() => {
      ...
      store.placeBrick({ partId: 'brick-2x4', color: 'blue', x: 5, y: 3, z: 5, rot: 1 })
    })

    let count = await page.evaluate(() => Object.keys(window.__blockyStore!.getState().bricks).length)
    expect(count).toBe(2)
    ...
    count = await page.evaluate(() => Object.keys(window.__blockyStore!.getState().bricks).length)
    expect(count).toBe(0)
