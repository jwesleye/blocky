...
describe('snapHitToGridCell', () => {
  it('snaps ground-plane hit to nearest stud cell with y=0', () => {
    ...
    expect(snapHitToGridCell({ point: { x: 0, y: 0, z: 0 } })).toEqual({ x: 0, y: 0, z: 0 })
    
    // hit at (8.1, 0, 16.4) -> grid cell (1, 0, 2)
    expect(snapHitToGridCell({ point: { x: 8.1, y: 0, z: 16.4 } })).toEqual({ x: 1, y: 0, z: 2 })
  })

  it('snaps brick-top hit to grid cell with matching y', () => {
    // hit at (8, 9.6, 16) with faceY = 3 (plates)
    expect(snapHitToGridCell({ point: { x: 8, y: 9.6, z: 16 }, faceY: 3 })).toEqual({ x: 1, y: 3, z: 2 })
  })
  ...
  it('returns null for off-baseplate points', () => {
    ...
    expect(snapHitToGridCell({ point: { x: maxBound + 1, y: 0, z: 10 } })).toBeNull()
  })
})
