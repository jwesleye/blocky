...
vi.mock('@react-three/drei', () => ({
  Instances: (props: { limit?: number; [key: string]: unknown }) => (
    <group data-testid="Instances" limit={props.limit} {...props} />
  ),
  Instance: (props: { [key: string]: unknown }) => (
    <group data-testid="Instance" {...props} />
  ),
}))
...
describe('InstancedBricks', () => {
  ...
  it('groups bricks into Instances by part and color', async () => {
    ...
    const instancesGroups = renderer.scene.findAll(
      (node) => node.props['data-testid'] === 'Instances',
    )
    ...
    const [redGroup, blueGroup] =
      group1.props.limit === 2 ? [group1, group2] : [group2, group1]

    expect(redGroup.props.limit).toBe(2)
    const redInstances = redGroup.findAll(
      (node) => node.props['data-testid'] === 'Instance',
    )
    expect(redInstances).toHaveLength(2)
    const redMaterial = redGroup.findAll(
      (node) => node.type === 'MeshStandardMaterial',
    )[0]
    expect(redMaterial.props.color).toBe('#ff0000')
    const redGeometry = redGroup.findAll(
      (node) => node.props.attach === 'geometry',
    )[0]
    expect(redGeometry.props.object.partId).toBe('brick-2x4')

    expect(blueGroup.props.limit).toBe(1)
    const blueInstances = blueGroup.findAll(
      (node) => node.props['data-testid'] === 'Instance',
    )
    expect(blueInstances).toHaveLength(1)
    const blueMaterial = blueGroup.findAll(
      (node) => node.type === 'MeshStandardMaterial',
    )[0]
    expect(blueMaterial.props.color).toBe('#0000ff')
    const blueGeometry = blueGroup.findAll(
      (node) => node.props.attach === 'geometry',
    )[0]
  ...
  it('provides geometry builder with base grid units instead of scaled scene units', async () => {
    ...
    const instancesGroup = renderer.scene.findAll(
      (node) => node.props['data-testid'] === 'Instances',
    )[0]
    // Get the primitive element that has the geometry attached
    const geometry = instancesGroup.findAll(
      (node) => node.props.attach === 'geometry',
    )[0]
