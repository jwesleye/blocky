...
describe('PartPicker', () => {
  it('renders the default selected part as checked', () => {
    ...
    expect(screen.getByRole('radio', { name: 'Brick 2×4' })).toHaveAttribute('aria-checked', 'true')
  })
  ...
  it('renders a button for every part in the catalog', () => {
    ...
    expect(screen.getByRole('radio', { name: 'Round Brick 1×1' })).toBeInTheDocument()
  })
  ...
  it('marks non-selected parts as unchecked', () => {
    render(<PartPicker selected="brick-2x4" onSelect={() => undefined} />)
    expect(screen.getByRole('radio', { name: 'Brick 1×1' })).toHaveAttribute('aria-checked', 'false')
  })

  it('groups parts by type with accessible radiogroup labels', () => {
    render(<PartPicker selected="brick-2x4" onSelect={() => undefined} />)
    expect(screen.getByRole('radiogroup', { name: 'Bricks' })).toBeInTheDocument()
    expect(screen.getByRole('radiogroup', { name: 'Plates' })).toBeInTheDocument()
    expect(screen.getByRole('radiogroup', { name: 'Tiles' })).toBeInTheDocument()
    expect(screen.getByRole('radiogroup', { name: 'Slopes' })).toBeInTheDocument()
    expect(screen.getByRole('radiogroup', { name: 'Rounds & Specials' })).toBeInTheDocument()
  })
})
